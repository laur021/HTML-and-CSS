...
app.put("/jokes/:id", async (req, res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === updatedJoke.id);
...
app.patch("/jokes/:id", async (req, res) => {
  const currentJoke = jokes.find((joke) => joke.id === parseInt(req.params.id));
  const updatedJoke = {
    id: currentJoke.id,
    jokeText: req.body.jokeText || currentJoke.jokeText,
    jokeType: req.body.jokeType || currentJoke.jokeType
  };
  //need pala to kasi para mahanap yung index, ang mali ko ginagwa kong index yung id
  const searchIndex = jokes.findIndex((joke) => joke.id === currentJoke.id);
  jokes[searchIndex] = updatedJoke;
  res.json(updatedJoke);
});
